...
function extractFundraisingData(html) {
  ...
  const dollars = candidates.flatMap(t => t.match(/\$\s*[\d,]+(?:\.\d{2})?/g) || [])
    .map(n => parseFloat(n.replace(/[\$\s,]/g, ''))).filter(Number.isFinite);
...
app.post('/api/organizations', (req, res) => res.status(405).json({ error: 'Adding organizations is disabled (hard-wired list).' }));
app.delete('/api/organizations/:id', (req, res) => res.status(405).json({ error: 'Deleting organizations is disabled (hard-wired list).' }));
